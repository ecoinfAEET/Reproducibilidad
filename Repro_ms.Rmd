---
title: "| Ciencia reproducible: qué, por qué y cómo \n| Reproducible science: what, why and how\n"
...
La ciencia se caracteriza por seguir unas pautas metodológicas que garantizan su validez epistemológica [@Pigliucci2013g], es decir...¿? [Paco, me traduces epistemologica??](). Los artículos científicos tienen una sección de métodos explicando los pasos seguidos en la recolección y análisis de datos. Esta información resulta crucial para evaluar racionalmente la veracidad y robustez de las conclusiones del artículo, así como para permitir futuras repeticiones del estudio por otros autores. Sin embargo, en la mayoría de ocasiones las escuetas descripciones verbales que aparecen en la sección de métodos resultan insuficiente para conocer todos los detalles de los análisis [@Ince2012] (**Fig. 1)**, y este problema resulta cada vez más acuciante con el aumento de la complejidad en los análisis estadísticos [@Michener2012].

Un estudio científico es reproducible si el texto del artículo viene acompañado de código (texto interpretable por un ordenador) que permite recrear exactamente todos los resultados y figuras incluidos en el artículo a partir de los datos originales [@Marwick2016]. El concepto es por tanto diferente al de repetibilidad, que se refiere a la posibilidad de replicar el mismo estudio (con nuevos datos) a partir de la información proporcionada en el artículo. La reproducibilidad se relaciona principalmente con la transparencia, trazabilidad, y completitud [Pues, sí completitud esta en la RAE, pero me suena raro, yo lo quitaria]() del protocolo seguido para llegar a unos resultados concretos a partir de un conjunto de datos determinado. La reproducibilidad no es una cualidad binaria sino un gradiente que va desde trabajos totalmente irreproducibles (que sólo contienen el texto, tablas y figuras finales) a estudios perfectamente reproducibles donde la integración de texto, código y datos permite regenerar fácilmente el resultado final a partir de los datos originales (**Fig. 2**).[poner ejemplos?]()
...
Ningún estudio científico puede garantizar que sus resultados sean correctos, pero sí reproducibles [@Peng2011]. La reproducibilidad es por tanto una garantía de transparencia y calidad. En los últimos años ha aumentado la presión por incrementar la reproducibilidad de los trabajos científicos, sobretodo debido a la creciente detección de errores graves en artículos que carecen de garantías de reproducibilidad [@Alberts2015b; @NatureEditorial2014a; @NatureEditorial2014b]. De hecho, el número de revistas y fuentes de financiación que requieren la publicación de datos y código cada día es mayor [@Stodden2013]. 
...
Aquí (https://github.com/Pakillo/rmdTemplates) puede descargarse una plantilla para escribir artículos en Rmarkdown para la revista Ecosistemas. [yo pasaría esto a la parte de recursos porque es un recurso para trabajar con markdown][NB: Yo lo dejaria en esta sección, ya que es como un reto que dice: "escribe tu proximo articulo para ecosistemas en rmarkdown!"]
...
La reproducibilidad de los resultados científicos es una cualidad inherente a la ciencia de calidad. Hacer ciencia de manera reproducible no sólo es un requisito creciente en muchas revistas y fuentes de financiación sino que además aporta muchas ventajas para el investigador. A la larga, la ciencia reproducible ahorra tiempo y esfuerzo, reduce el riesgo de cometer errores y aumenta el impacto y utilidad de los trabajos en la comunidad científica. Desarrollar estudios plenamente reproducibles requiere un esfuerzo inicial de aprendizaje, pero la transición puede hacerse gradualmente [hecho a faltar una hoja de ruta que explique por donde empezar, quizas se puede poner más arriba, en como hacer ciencia reproducible o aquí. Algo como así: "La mejor manera para empezar el camino reproducible es usar *scripts*, por ejemplo de R, para manipular y analyzar los datos y hacerlos públicos. Una vez conseguido esto, empieza ha utilizar documentos dinamicos como Rmarkdown y luego aprende a usar control de versiones. Intentar hacerlo todo a la vez puede ser contraproducente, pero a paso a paso no es difíl!"]() y los frutos se recogen desde el primer día. Esperamos que este artículo anime a muchos investigadores a adoptar estas prácticas, y las pautas y recursos aquí recogidos les faciliten dicha transición.
...
**Figura 1**. Habria que pedirle permiso al autor... No se si nos lo dara [Nacho: Por mi podemos prescindir de ella] http://www.sciencecartoonsplus.com/index.php

**Figura 2**. Habría que redibujar esta figura [voluntarios?], y pedir permiso a Science. [yo puedo hacerlo!]
...
**Figure 5**. Version control al estilo PhDcomics. [pedir permiso. Nacho: O redibujar?]

**Figure 6**. Ejemplo de historia de git/github. [usar captura de pantalla de github, o pintar commit history? Nacho: Usar el History de Rstudio??]
...
**Figura 8**. Red de dependencias externas de un proyecto (incluir?)[Nacho. Me parece bien, pero no se si es mejor una red más pequeña, que no asuste tanto?]
