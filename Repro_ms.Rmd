...
Aunque en este articulo nos centramos en como las nuevas tecnologias nos permiten hacer nuestro workflow reproducible desde que tenemos los datos hasta que publicaos el paper, no hay que olvidar que el proceso empieza mucho antes con la toma de datos y antes aun con el diseño del proyecto. 

El propio escrito con el proyecto inicial, no solo tiene el valor de conseguir financiación para el proeyecto, sino que nos sirve de guia. Diversos autores estan tomando la iniciativa de publicar abiertamente estos proyectos para asegurar la transparencia del proceso, para legitimizar la atribución de las ideas contenidas, y sobretodo, para que quede un registro de nuestras intenciones que se puedan confrontar con los resultados obtenidos (E. White webpage). Esto es importante para evitar p-hacking (ref and definition), y forma parte de un workflow reproducible y abierto (¿?). De hecho, en otras disciplinas, el pre-registro de experimentos es una practica habitual (ref). --quizas este parrafo sobra, pero de momento lo he colado aquí-- En qualquier caso, escribir protocolos detallados antes de coger los datos, y actualizarlos es el primer paso para poder reproducir el proceso. Esto lleva tiempo, y es tentador explicar verbalmente que datos vamos a tomar al equipo de trabajo, pero es la unica forma de dejar un registro, y no olvidar nada... Preparar tablillas para entrar datos en campo tambien asegura que no nos dejaremos ningun dato relevante, y estas tablillas podran ser re-usadas ( o adaptadas) en futuros proyectos, ahorrando tiempo entonces.

Automatizar la toma de datos para que sea reproducible es algo más complicado, y variara enormemente segun el tipo de datos con los que trabajemos. De todas maneras hay algunas practicas más o menos comunes que pueden servir de guia. El primero es que las muestras físicas, cuando las haya, y los datos digitales siempre han de estar conectados. Esto hara que qualquier dato pueda ser verificado (mayormente por nosotros mismos) en el futuro. Preservar estas muestras físicas en el tiempo es de vital importancia, pero tremendamente dificil, ya uqe normalmente no hay presupuesto para ello, y dependiendo se su naturaleza requieren de costosa preservación y/o ocupan gran volumen. En el caso de que no haya espacio especimenes debidamente identificados pueden donarse a museos, etc?. 

Cuando los datos se toman directamente en campo, es importante que estos sean reproducibles. Es decir, que se tomen de manera objetiva (numero de flores) y no subjetiva (porcentage de corvetura estimado) y que no haya un sesgo por observador (datos que mejoren con la practica). En el caso que se necesiten datos subjetivos, es importante que cotejar quan reproducible es el resultado, comparando medidas identicas por el mismo observado o entre observadores. Por ejemplo, en biologia del comportamiento, donde se suelen medir comportamientos, es comun  reportar reproducibilidad en medir estos comportamientos (e.g. registrados en video), pero esto aplica tambien a datos más simples, donde tendemos a confiar en que no haya sesgo. Este tipo de resultados deben ir en el material suplementario (e.g. Winfree papers). Otra practica realacionada es medir los datos de menera "ciega" siempre que sea posible, ya que el sesgo inconsciente que podemos introducir es muy grande (ref).

La parte de toma de datos sigue confiandose a la breve reseña expuesta en el material y metodos, sin embargo, apoyar esta explicación con tests propios de reproducibilidad en su obtención, y dar los detalles de forma rigurosa pueden significar un cambio. Por un lado, aportar estos datos contribuyen a dar confianza al lector en tus resultaod, por otro, incluir detalles de que protocolos no funcionaron, o se hubieron de adaptar, puede salvar muchas horas a alguin interesado en repetir esa clase de muestreo.

Entrar datos es una de las partes más tediosas y aparantemente fáciles, pero sorprendentemente es muy propensa a generar errores "tontos". Usar bases de datos con campos fijos ayuda a minimizar estos errores. Este tambien es el momento de crear los metadatos, ya que es cuando estan frescos en la memoria y el coste es bajo. EXPLICAR QUE SON Y POR QUE ASEGURAN REPRODUCINBILIDAD. Si no lo haces ahora, lo más probable es que no lo hagas nunca, ya que cuando ha acabado el proyecto no recuerdas bien las cosas, y "da mucho traajo". Sin embargo, tener bien documentado los datos no solo es bueno para compartir datos, sino como recalcamos en este articulo, para ti mismo en tres meses vista. Si usas R, una alternativa es la libreria EML ... Añadir algo sobre tidy data? quizas solo la ref. 
...
Todo análisis reposa sobre plataformas o paquetes de software que suelen evolucionar con el tiempo. Así, cambios en paquetes de R, o la misma versión de R base, pueden hacer que nuestro análisis deje de ser reproducible en poco tiempo (semanas, meses), incluso habiendo seguido los protocolos de reproducibilidad esbozados más arriba. De manera que, para asegurar que nuestro proyecto siga siendo reproducible en el futuro, necesitamos (i) registrar las dependencias externas de nuestro análisis (e.g. paquetes de R), y (ii) asegurar de alguna forma el acceso a versiones pasadas de dicho software. Para el primer punto, es tan sencillo como ejecutar `sessionInfo()` al finalizar el análisis en `R`. Este comando devuelve una lista de todos los paquetes utilizados y su versión (Figura?). El segundo punto es algo más complejo, aunque está siendo abordado por varios frentes que difieren significativamente en cuanto a facilidad de manejo. Entre los más sencillos se encuentran `rctrack` (ref) o `checkpoint` (ref). `rctrack` es un paquete de R que se encarga de almacenar automáticamente todos los datos y funciones utilizados por un script de R. Es tan sencillo de usar como escribir `begin.rctrack()` al comienzo del script, y `end.rctrack()` al final. De esta manera, `rctrack` se encarga de generar un fichero RData o zip con todos los materiales para que el proyecto sea siempre reproducible al almacenar in situ todas las dependencias externas. 

`checkpoint` es otro paquete de R recientemente desarrollado e igualmente fácil de usar que, al contrario que `rctrack`, no almacena el código externo in situ, sino que descarga los paquetes de R utilizados por nuestro script desde un servidor que almacena imágenes diarias de CRAN desde Septiembre de 2014. La versión concreta a descargar de cada paquete se determina por la fecha. Una posible limitación es que se limita a paquetes incluidos en CRAN (i.e. excluye paquetes instalados desde GitHub u otro repositorio externo). `packrat` es una alternativa desarrollada por RStudio que, en la línea de rctrack, almacena una copia local de todos los paquetes utilizados en un proyecto (incluyendo aquellos instalados desde GitHub). Su integración con RStudio lo hace fácil de utilizar. Finalmente, `drat` y `rocker`...
